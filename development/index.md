...
| Voxel space of a simulated dataset | `space: str = 'mni'` on `load_haxby_example` — which grid the simulated data live on: `'mni'` is the package's 3 mm MNI152 mask, so every plotting method and mask operation works; `'grid'` is the 10 x 10 x 5 synthetic volume kept for tests that need construction in milliseconds |
| Block order across simulated runs | `block_order: str = 'independent'` on `load_haxby_example` — whether the runs share a condition order: `'independent'` draws a fresh order per run; `'shared'` draws one order and gives it to every run, so the runs line up TR by TR for intersubject correlation and alignment. Noise differs per run either way |
...
| Result export directory | `directory` on `FitResult.write` and `ContrastResult.write` — a folder the record fills with one NIfTI per map plus a design CSV and a JSON sidecar, created when it does not exist. `BrainData.write` and `DesignMatrix.write` keep `file_name` because each writes exactly one file |
| Exported filename prefix | <code>prefix: str &#124; None = None</code> on `FitResult.write` and `ContrastResult.write` — prepended to every written name as `<prefix>_`; `None` writes the bare names |
