...
| Decoding and PredictResult | Brain_Data prediction methods return older result structures | Retain BrainData whole-brain, ROI and searchlight decoding and its result contract. Current PredictResult fields still differ from the approved mode/shape rules. | [BrainData prediction contract](specs/braindata.md#prediction-and-decoding), `nltools/data/results.py::PredictResult`, `nltools/data/braindata/prediction.py`. Keep PredictResult; separate collection-only result fields/helpers from the broader approved result redesign. |
...
| Defer result container | `PredictCollection` in `nltools/data/results.py` and `nltools/data/__init__.py` | Aggregates collection decoding. Preserve the adjacent BrainData `PredictResult` record. |
...
| Review collection-only fields | `PredictResult.permutation_scores`, `PredictResult.permutation_pvalue` | Current producers belong to collection group prediction. Check callers and the approved PredictResult contract before removal; do not remove general inference algorithms. |
