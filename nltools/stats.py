...
def isps(data, sampling_freq=.5, low_cut=.04, high_cut=.07, order=5, pairwise=False):
    ...
    phase = np.angle(hilbert(_butter_bandpass_filter(pd.DataFrame(data), low_cut, high_cut, sampling_freq, order=order), axis=0))

    if pairwise:
        phase = np.array([phase[:,i] - phase[:,j] for i in range(phase.shape[1]) for j in range(phase.shape[1]) if i < j]).T
