...
class TestLocalAlignmentEdgeCases:
    ...
    def test_piecewise_not_implemented(self, sample_multisubject_data, small_mask):
        """Test that piecewise scheme raises NotImplementedError."""
        from nltools.algorithms.alignment import LocalAlignment

        la = LocalAlignment(scheme="piecewise")

        with pytest.raises(NotImplementedError, match="(?i)piecewise"):
            la.fit(sample_multisubject_data, small_mask)
