...
**Layer two, for users.** The facade classes with their methods, plus a short designated list of standalone functions. Users are never expected to call an estimator, a delegate, or a helper directly; a return object (`FitResult`, `PredictResult`, `BootstrapResult`, `ContrastResult`, `BrainSpaceConfig`) is data they read, not something they construct — `FitResult.write` and `ContrastResult.write` save one to disk and are the only behaviour any of them carries. A public-looking name does not make something user-facing; being listed here does.
...
`nltools.data` | the six classes and the five result types
...
Marimo notebooks under `docs/tutorials/` are the tutorial sources. Edit the `.py`; the `.md` sibling is generated by `docs-generate` and not committed. `docs/changelog.md` is generated the same way, by git-cliff from git history, and is not committed either; a release tag is what gives its Unreleased section a version heading, and the docs deploy checks out full history so the tags are visible. `docs-build` executes every notebook and fails on a cell that raises or warns; `docs-serve` replays recorded outputs for notebooks whose cells did not change.

Zensical is the only docs toolchain. `zensical.toml` configures the site; `docs-build` is the strict build, `docs-build-fresh` is the cold build the deploy runs, and `docs-clean` drops `site/`, zensical's page cache and the generated tutorial pages.
