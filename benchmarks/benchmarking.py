...
Examples:
  ...
  %(prog)s --no-gpu
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Show benchmark plan without running"
    )

    parser.add_argument(
        "--no-gpu", action="store_true", help="Skip GPU benchmarks (CPU only)"
    )
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="Minimal output, no progress bars"
    )
...
def estimate_runtime(
    n_samples, n_voxels, n_features, estimation_style, cv_folds=5, cv_alphas=10
):
    """
    ...
    f_factor = (
        n_features / reference_features
    )  # Linear in features for small n_features
...
def print_dry_run_summary(config):
    ...
    for i, (n_samples, n_voxels, n_features, style) in enumerate(conditions, 1):
        ...
        style_short = (
            "estimates"
            if style == "estimates_only"
            else f"fit ({config['cv_folds']}-CV)"
        )
        row = (
            f"{i}/{len(conditions):<4} {n_samples:<8} {n_voxels:<10,} {n_features:<10} {style_short:<13} "
...
def run_systematic_benchmarks(config=None) -> pd.DataFrame:
    ...
    estimation_styles = [
        (style, style_map.get(style, style)) for style in config["estimation_styles"]
    ]
    ...
    total_conditions = (
        len(n_samples_options)
        * len(n_voxels_options)
        * len(n_features_options)
        * len(estimation_styles)
    )
        ...
        for n_voxels, voxels_label in n_voxels_options:
            for n_features, features_label in n_features_options:
                for est_style, est_label in estimation_styles:
                    ...
                    if pbar:
                        pbar.set_description(
                            f"Cond {condition_num}/{total_conditions}: n={n_samples}, v={n_voxels:,}, f={n_features}, {est_style[:3]}"
                        )
                    ...
                    if not config.get("quiet", False) and not pbar:
                        print(
                            f"  Generating data: X={n_samples}×{n_features}, y={n_samples}×{n_voxels:,}..."
                        )
                ...
                if not skip_gpu:
                    ...
                    if not config.get("quiet", False) and not pbar:
                        print(
                            f"{time_torch:.2f}s (memory: {mem_torch:+.1f} MB, speedup: {speedup:.2f}x)"
                        )
...
def main():
    ...
    try:
        ...
        print(
            f"Error: Invalid samples argument '{args.samples}'. Must be comma-separated integers."
        )
        return
    ...
    try:
        ...
        print(
            f"Error: Invalid voxels argument '{args.voxels}'. Must be comma-separated integers."
        )
        return
    ...
    try:
        ...
        print(
            f"Error: Invalid features argument '{args.features}'. Must be comma-separated integers."
        )
        return
    ...
    try:
        ...
        for x in args.estimation.split(","):
            ...
            if style not in estimation_map:
                print(
                    f"Error: Invalid estimation style '{style}'. Must be 'estimates' or 'fit'."
                )
                return
