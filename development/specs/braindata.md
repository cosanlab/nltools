...
independently owned. `BrainData(other)` is one of them: the inherited mask, the
voxel resolution, and inherited `.X`/`.Y` frames are copied, so neither object
can mutate the other's state. An explicit `mask` argument is the caller's own
image, as on every other constructor path.
