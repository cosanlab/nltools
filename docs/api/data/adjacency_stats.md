...
[`ttest`](#data-adjacency-stats-ttest) | Run a one-sample t-test across stacked matrices.
...
```python
ttest(adj, *, popmean = 0.0, permutation = False, n_permute = 5000, tail = 2, return_null = False, n_jobs = -1, random_state = None, progress_bar = False)
```
...
Name | Type | Description | Default
---- | ---- | ----------- | -------
`adj` | <code>[Adjacency](#page-data-adjacency)</code> | Stack of two or more matrices with the same node order and storage kind. | *required*
...
`tail` | <code>int \| str</code> | `2`/`'two'` (two-tailed, default) or `1`/`'one'` (one-tailed: mean > `popmean`). | <code>2</code>
...
Type | Description
---- | -----------
<code>ValueError</code> | If `adj` holds fewer than two matrices.
