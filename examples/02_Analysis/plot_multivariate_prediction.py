...
# ---------------------------------------------------------
#
# We can now predict the The output variable is a dictionary of the most 
...
svr_stats = dat.predict(algorithm='svr', 
                        cv_dict={'type': 'kfolds','n_folds': 5, 'n':len(dat.Y)}, plot=False,
                        **{'kernel':"linear"})

pcr_stats = dat.predict(algorithm='pcr', 
                        cv_dict={'type': 'kfolds','n_folds': 5, 'n':len(dat.Y)}, plot=False)

lasso_stats = dat.predict(algorithm='lasso', 
                        cv_dict={'type': 'kfolds','n_folds': 5, 'n':len(dat.Y)}, plot=False,
                        **{'alpha':.1})
...
subject_id = dat.X['SubjectID']
ridge_stats = dat.predict(algorithm='ridge', 
...
ridgecv_stats = dat.predict(algorithm='ridgeCV', 
                        cv_dict={'type': 'kfolds','n_folds': 5, 'stratified':dat.Y}, 
