...
<https://academic.oup.com/cercor/article/26/6/2919/1754308>`, which uses an
...
<http://haxbylab.dartmouth.edu/publications/HGC+11.pdf>`. This technique is
implemented in the `PyMVPA Toolsbox <http://www.pymvpa.org/>`. Another promising
functional alignment technique known as `Shared Response Model
<http://papers.nips.cc/paper/5855-a-reduced-dimension-fmri-shared-response-model.pdf>`
 ...
 implemented in the `brainiak toolbox <https://github.com/brainiak/brainiak>`.
...
# data into the original subject specific voxel space.
...
[a[i,0].set_ylabel(x,fontsize=16) for i,x in enumerate(['Original Voxels','Aligned Voxels', 'Backprojected Voxels'])]
