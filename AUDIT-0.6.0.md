...
#### 🟡 F073 · MEDIUM — partially implemented (`scheme`→`spatial_scale` done; `progress_bar` now wired through to LocalAlignment. `cache=` KEPT: it is a SPEC-mandated uniform lazy-knob on all BrainCollection-returning methods — test_construction asserts its presence — so removing it is wrong; its no-op-for-align behavior is a separate SPEC-level decision, not a stray kwarg.)
